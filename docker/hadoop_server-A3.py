...
def run_job():
    ...
    logger.mark("Received Job Request.")
    ...
    if "A1" in ASSIGNMENT_ID or "A2" in ASSIGNMENT_ID:
        ...
        if "T1" in ASSIGNMENT_ID:
            SPARK_FILE = request.form["spark"]
            logger.mark(f"Starting Spark Job. Team ID : {TEAM_ID} Assignment ID : {ASSIGNMENT_ID} Submission ID : {SUBMISSION_ID}")
            job_result = run_spark_job(TEAM_ID, ASSIGNMENT_ID, SUBMISSION_ID, TIMEOUT, SPARK_FILE)
        elif "T2" in ASSIGNMENT_ID:
            PRODUCER = request.form["producer"]
            CONSUMER = request.form["consumer"]
            logger.mark(f"Starting Kafka Job. Team ID : {TEAM_ID} Assignment ID : {ASSIGNMENT_ID} Submission ID : {SUBMISSION_ID}")
            job_result = run_kafka_job(TEAM_ID, ASSIGNMENT_ID, SUBMISSION_ID, TIMEOUT, PRODUCER, CONSUMER)

    logger.mark(f"Job Completed. Team ID : {TEAM_ID} Assignment ID : {ASSIGNMENT_ID} Submission ID : {SUBMISSION_ID}\n")
...
def run_spark_job(team_id, assignment_id, submission_id, timeout, spark:str):
    """
    Arguments
    ---------
    ...
    spark : List of spark codes. Each element represents mapper for ith task (Update needed)
    """
    ...
    with open(os.path.join(task_path, "spark.py"), "w+") as f:
        f.write(spark)
    
    logger.mark(f"Created spark.py at {os.path.join(task_path, 'spark.py')}")
    ...
    if assignment_id == "A3T1":
        ...
        process_exit_code = spark_process.wait()
        ...
        if flag and current_job_state == "SUCCEEDED":
        ...
        elif flag and current_job_state == "FAILED":
            application_id = current_job["id"]
...
def run_kafka_job(team_id, assignment_id, submission_id, timeout, producer:str, consumer:str):
    """
    Arguments
    ---------
    ...
    assignment_id : Assignment ID of the submission
    timeout : Timeout in seconds 
    producer : List of spark codes. Each element represents mapper for ith task (Update needed)
    consumer : List of spark codes. Each element represents mapper for ith task (Update needed)
    """
    ...
    with open(os.path.join(task_path, "producer.py"), "w+") as f:
        f.write(producer)

    with open(os.path.join(task_path, "consumer.py"), "w+") as f:
        f.write(consumer)
    
    logger.mark(f"Created Producer.py at {os.path.join(task_path, 'producer.py')}")
    logger.mark(f"Created Consumer.py at {os.path.join(task_path, 'consumer.py')}")

    st = os.stat(os.path.join(task_path,"producer.py"))
    os.chmod(os.path.join(task_path,"producer.py"), st.st_mode | stat.S_IEXEC)

    st = os.stat(os.path.join(task_path,"consumer.py"))
    os.chmod(os.path.join(task_path,"consumer.py"), st.st_mode | stat.S_IEXEC)
    ...
    if assignment_id == "A3T2":
        logger.mark(f"Spawning Kafka Process")
...
def cleanup(team_id, assign_id, task) -> int:
    ...
    for files in os.listdir(task_path):
        ...
        if os.path.isdir(filename):
            for f_ in os.listdir(filename):
                f_name = os.path.join(filename, f_)
                os.remove(f_name)
...
def initialize_environment(add_dataset=True) -> Tuple:
    ...
    spark_start = subprocess.Popen([f"bash /opt/spark/sbin/start-all.sh"], shell=True, text=True)
    _ = spark_start.wait()

    print("Spark Started")

    zookeeper_process = subprocess.Popen(["/usr/local/kafka/bin/zookeeper-server-start.sh /usr/local/kafka/config/zookeeper.properties > zookeeper.log 2>&1 &"], shell=True, text=True)
    _ = zookeeper_process.wait()

    time.sleep(5)

    kafka_process = subprocess.Popen(["/usr/local/kafka/bin/kafka-server-start.sh /usr/local/kafka/config/server.properties > kafka.log 2>&1 &"], shell=True, text=True)
    _ = kafka_process.wait()
...
if __name__ == "__main__":
    ...
    if return_code == 0:
        print("Environment has been setup successfully!")
